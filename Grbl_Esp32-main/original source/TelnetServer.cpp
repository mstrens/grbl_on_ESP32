...
#    endif
            ...
            if (i >= MAX_TLNT_CLIENTS) {
				...
				if ( millis() - _lastPush > 2000){ // Added by MStrens : when no data have been received since 2 seconds and a new client comes, priority is given to new client
					_telnetClients[0].stop(); // Added by MStrens :
					_telnetClients[0] = _telnetserver->available(); // Added by MStrens :
				}	
				//Serial.println("New telnet client rejected") ;// added by ms for testing !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                //_telnetserver->available().stop(); // commented by MStrens
            }
        }
    }
